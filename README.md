...
Build the image from the repository root because the Dockerfile lives under `docker/`:
```Bash
docker build -f docker/Dockerfile -t ground_vio:catkin .
```

The image installs the native dependencies used by the project, including **Eigen** and **OpenCV**, plus the ROS packages needed by `ground_tracker_node` such as `cv_bridge`, `image_transport`, `tf`, `sensor_msgs`, `nav_msgs`, and `rviz`.

If you want to start the prepared runtime container, use:
```Bash
./docker/run.sh /path/to/datasets /path/to/depth
```
